        ...
        static async Task Main(string[] args)
        {
            ...
            using (var math = new MathematicaHelper())
            {
                try
                {
                    ...
                    // ====================================================================
                    // 示例 2: 单个整数参数，返回数值（阶乘）
                    // 注意：已更名为 ComputeFactorial 以避免冲突
                    // ====================================================================
                    Console.WriteLine("【示例 2】单个整数参数，返回数值（阶乘）");
                    Console.WriteLine("-".PadRight(80, '-'));
                    
                    int num2 = 6;
                    int factorialResult = math.ExecuteForInteger($"MathematicaFunctions`ComputeFactorial[{num2}]");
                    Console.WriteLine($"输入: {num2}");
                    Console.WriteLine($"ComputeFactorial({num2}) = {factorialResult}");
                    Console.WriteLine();
                    ...
                    var task1 = math.ExecuteForIntegerAsync("MathematicaFunctions`ComputeFactorial[10]");
                    ...
                    Console.WriteLine($"任务1结果 (ComputeFactorial(10)): {task1.Result}");
                    ...
                    // ====================================================================
                    // 示例 17: 生成 3D 图像并保存为文件 (新增)
                    // ====================================================================
                    Console.WriteLine("【示例 17】生成 3D 图像并保存为 JPG 文件");
                    Console.WriteLine("-".PadRight(80, '-'));

                    // 构造绘图命令，或者调用 .m 包中的 GenerateSamplePlot[]
                    ...
                    try 
                    {
                        ...
                        // 调用我们最新的“文件桥接”方法获取图片字节流
                        byte[] imageBytes = await math.ExecuteForImageBytesAsync(plotCmd, "JPG");
                        Console.WriteLine($"图像生成成功！大小: {imageBytes.Length} 字节");
                        ...
                        Console.WriteLine($"图像已保存至: {filePath}");
                    }
                    catch (Exception ex)
                    {
